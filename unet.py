...
from data_process import count_cell, merge
...
class Unet(object):
    _defaults = {
        ...
        #-------------------------------------------------------------------#
        "model_path"    : 'logs/ep100-loss0.055-val_loss0.112.pth',
        #--------------------------------#
        #   所需要区分的类的个数+1
        #--------------------------------#
        "num_classes"   : 3,
    ...
    #   检测图片
    #---------------------------------------------------#
    def detect_image(self, img, count=False, name_classes=3):
        ...
        if self.mix_type == 0:
            ...
            # 通过设置阈值，将预测结果转换为二值图
            # 统计不同的像素值种类和对应的频次
            unique_values, counts = np.unique(pr, return_counts=True)
            print(unique_values)
            binary_img = (pr == 2).astype(np.uint8) * 255 #二值，并对特定像素值进行255,其他0

            # #------------------------------------------------#
            # #   将新图片转换成Image的形式
            # #------------------------------------------------#
            #image   = Image.fromarray(np.uint8(seg_img))
            old_image = Image.fromarray(np.uint8(binary_img))
            # #------------------------------------------------#
            # #   将新图与原图及进行混合,并计数
            # #------------------------------------------------#
            #image   = Image.blend(old_img, image, 0.7)
            result_string,merge_image=count_cell(binary_img,image)
